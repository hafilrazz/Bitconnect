# Internet E2E messaging (Netless)
...
- **Netless ID** = recipient’s static X25519 public key (64 hex chars).
...
- Metadata: that *someone* sent a Netless event at time T (relay sees size/time/tags).
- Your Netless ID is a public address (like a phone number for crypto).

## How to use

1. Open **Internet E2E DM** (globe icon).
2. Copy **Your Netless ID** and send it out-of-band (QR, SMS, etc.).
3. Paste their Netless ID → **Save & chat**.
4. Connect (cloud icon) and send. Messages show 🔒.
